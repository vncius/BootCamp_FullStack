...
function calculateSalaryFrom(salarioBruto) {
  ...
  const descontoINSS = calculateDiscountINSS(baseINSS);

  const baseIRPF = baseINSS - descontoINSS;
  const descontoIRPF = calculateDiscountIRPF(baseIRPF);
  const salarioLiquido = baseINSS - descontoINSS - descontoIRPF;

  return {
    baseINSS: formatNumber(baseINSS),
    descontoINSS: formatNumber(descontoINSS),
    baseIRPF: formatNumber(baseIRPF),
    descontoIRPF: formatNumber(descontoIRPF),
